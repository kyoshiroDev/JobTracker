...
export class CandidaturesService {
  protected _annonces = signal<Candidature[]>([
    {
      id: 1,
  ...
  getAll(): Signal<Candidature[]> {
    return computed(() => this._annonces().sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime()));
  }
  ...
  readonly filteredAnnonces = computed(() => {
    ...
    return this._annonces().filter((annonce) => {
      ...
      const salaryMatch = filter.salary ? annonce.content.salary?.toString() === filter.salary.toString() : false;
  ...
  addAnnonce(formDataAnnonce: Omit<Candidature, 'id'>) {
    const id = 5;
    const newAnnonce: Candidature = {
      ...formDataAnnonce,
      id: id + 1,
