...
import { CandidatureForm, CompanyForm, ContentForm } from './candidatureForm';
...
@Component({
  selector: 'fdw-candidatures-form',
  ...
  template: `
    ...
    >
      <div class="w-[550px] max-h-[98vh] bg-JobTracker-white rounded-md z-3 border border-JobTracker-blue relative">
        <div class="flex justify-between items-center p-4">
          ...
          <p class="text-JobTracker-blue text-center text-lg md:text-2xl font-semibold justify-center flex-1">
            Nouvelle annonce
          </p>
          <button
        ...
        >
          <div class="flex flex-col w-full justify-center items-start gap-2 flex-nowrap">
            <label for="poste">Poste<span class="text-xs text-red-700">*</span></label>
            <input id="poste" type="text" formControlName="job" placeholder="Développeur web" class="" />
          </div>
          <!-- Entreprise -->
          @if (entrepriseForm()) {
          <fdw-entreprise-form [entrepriseForm]="formAnnonce.controls.company" />
...
export class CandidatureFormComponent {
  ...
  private readonly service = inject(CandidaturesService);
  ...
  protected readonly formAnnonce = this.fb.group<CandidatureForm>({
    job: this.fb.control('', Validators.required),
    company: this.fb.group<CompanyForm>({
      name: this.fb.control('', Validators.required),
      city: this.fb.control('', Validators.required),
      phone: this.fb.control('', [Validators.required, Validators.maxLength(10), Validators.pattern(/^[0-9]/)]),
    ...
    content: this.fb.group<ContentForm>({
      ...
      salary: this.fb.control('', [Validators.required, Validators.pattern(/^\d+$/)]),
      contractType: this.fb.control<'CDI' | 'CDD' | 'Freelance' | 'Stage' | null>(null, Validators.required),
      workMode: this.fb.control<'fullremote' | 'presentiel' | 'hybride' | null>(null, Validators.required),
  ...
  protected onSubmit() {
    ...
    const formDataAnnonce: Omit<Candidature, 'id'> = {
      job: form.job ?? '',
      company: {
