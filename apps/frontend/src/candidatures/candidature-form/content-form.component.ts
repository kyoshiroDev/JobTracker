...
import { ContentForm } from './candidatureForm';
...
@Component({
  ...
  template: `
    <fieldset class="flex flex-col gap-5 border border-JobTracker-blue p-5 rounded-md">
      <legend class="text-center font-semibold text-xl md:text-2xl px-2 text-JobTracker-blue">
      ...
      <div [formGroup]="contentForm()" class="flex flex-col gap-5">
        ...
        <div class="flex flex-col w-full justify-center items-start gap-2 flex-nowrap">
          ...
          <textarea id="about" rows="2" placeholder="Saisissez vôtre texte ici ..." formControlName="about"> </textarea>
        </div>
        <!-- Description -->
        <div class="flex flex-col w-full justify-center items-start gap-2 flex-nowrap">
          <label for="description">Descriptif<span class="text-xs text-red-700">*</span></label>
          <textarea id="description" rows="2" placeholder="Saisissez vôtre texte ici ..." formControlName="description">
          </textarea>
        </div>
        <!-- Skills -->
        <div class="flex flex-col w-full justify-center items-start gap-2 flex-nowrap">
          <label for="skills">Compétences<span class="text-xs text-red-700">*</span></label>
          <textarea id="skills" rows="2" placeholder="Saisissez vôtre texte ici ..." formControlName="skills">
          </textarea>
        </div>
        <!-- Benefits -->
        <div class="flex flex-col w-full justify-center items-start gap-2 flex-nowrap">
          <label for="benefits">Avantages<span class="text-xs text-red-700">*</span></label>
          <textarea id="benefits" rows="2" placeholder="Saisissez vôtre texte ici ..." formControlName="benefits">
          </textarea>
        </div>
        <div class="flex flex-wrap justify-start items-center gap-5">
          ...
          <div class="flex items-center justify-center gap-2">
            ...
            <input id="salary" type="text" placeholder="10000" formControlName="salary" />
            <p>annuelle</p>
          </div>
        ...
        <div class="flex flex-col w-full justify-left items-start gap-2 flex-wrap">
          ...
          <input id="annonceLink" type="text" placeholder="https://www.google.com" formControlName="annonceLink" />
        </div>
      </div>
    </fieldset>
  `,
