...
@Component({
  selector: 'fdw-candidatures-card',
  changeDetection: ChangeDetectionStrategy.OnPush,
  imports: [RouterLink],
  template: `
    <a
      [routerLink]="['/candidatures', annonce().id]"
    ...
    >
      <div class="inline-flex gap-4 w-full justify-between">
        <p class="text-lg md:text-xl text-JobTracker-blue font-semibold first-letter:uppercase">
          {{ annonce().job }}
        </p>
      ...
      <div class="w-full flex justify-start items-center text-sm md:text-lg text-JobTracker-blue font-semibold">
...
export class CandidatureCardComponent {
  ...
  readonly goToDetail = output<Candidature['id']>();

  readonly statusConfig: Signal<string | undefined> = computed(() => {
    const statusConfig = this.status.find((config) => config.label === this.annonce().content.status);
