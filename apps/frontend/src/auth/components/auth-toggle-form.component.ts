...
@Component({
  ...
  template: `
    <div class="px-5 pt-5">
      <div class="bg-secondary/50 rounded-md p-1 grid grid-cols-2 gap-1">
        <button type="button" (click)="toggleForm('login')" [class]="loginBtnClass()">Connexion</button>
        <button type="button" (click)="toggleForm('register')" [class]="registerBtnClass()">Inscription</button>
      </div>
    </div>
  `,
